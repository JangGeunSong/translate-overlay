...
export class RemoteInterpretationProvider implements LinguisticProvider {
  ...
  async interpret(context: InterpretationContext): Promise<InterpretationResult> {
    this.statuses.emit({
      ...
      mode: this.mode,
      state: "checking",
      message: "제품 문맥 해석 연결 확인 중",
    });
    const message = { type: "INTERPRET_CONTEXT", context } satisfies ReaderMessage;
    const response = await chrome.runtime.sendMessage(message) as InterpretationProviderResponse;
    if (!response?.ok) {
      this.statuses.emit({
        capability: "interpretation",
        mode: this.mode,
        state: "unavailable",
        message: "제품 문맥 해석을 사용할 수 없음",
      });
